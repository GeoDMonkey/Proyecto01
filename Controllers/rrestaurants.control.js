...
const getRestaurants = async (req, res) => {
    try {
        const restaurants = await Restaurants.findAll({
            where: { status: 'active' },
            include: [
                { model: Reviews, attributes: ['comment', 'riting'] },
            ],
            include: [
                {
                    model: Meals, attributes: ['name', 'price', 'status']
                }
            ]
        })
        res.status(200).json({
...
const postRestaurants = async (req, res) => {
    try {
        ...
        const createNewRestaurant = await Restaurants.create({ name, address, rating })
        res.status(201).json({
            status: 'Success',
            data: {
                createNewRestaurant
...
const updateReviewsId = async (req, res) => {
    ...
    const updateReview = await Reviews.findOne({ where: { id, status: "active" } });
