...
const getOrder = async(req, res)=>{
    try {
        const order = await Order.findAll({
            include:[
                {
                    model: Meals, attributes:['name','price', 'restaurantId', 'status' ]
                }
            ]
        })
        res.status(200).json({
...
const postOrder = async(req, res)=>{
    try {
        ...
        const mealPrice = await Meals.findOne({where: {status:'active',id: mealId}}) 
...
const updateOrder = async(req, res) =>{
    try {
        ...
        await order.update({mealId,userId:sessionuser.id, quantity, totalPrice: total })
        res.status(204).json({
            status: 'Success',
            data: {order}
        })
